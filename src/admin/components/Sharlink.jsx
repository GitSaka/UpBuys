...
const ShareLink = ({ slug}) => {
  ...
  const shareableLink = `localhost:5173/profile/coach/${slug}`;

  const copyToClipboard = () => {
    navigator.clipboard.writeText(`http://${shareableLink}`);
    setCopied(true);
  ...
  return (
    <div className="bg-purple-50 border border-purple-100 p-6 rounded-[32px] flex flex-col md:flex-row items-center justify-between gap-4 mb-10 animate-fadeIn">
      <div className="text-left">
        <h3 className="text-[10px] font-black uppercase text-purple-600 tracking-[0.2em] mb-1">Ton lien d'Empire est prêt 🚀</h3>
        <p className="text-sm font-bold text-gray-900 italic">https://{shareableLink}</p>
      </div>
      ...
      <div className="flex gap-2 w-full md:w-auto">
        ...
        <button 
          onClick={() => window.open(`http://${shareableLink}`, '_blank')}
